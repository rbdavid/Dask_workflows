...
if __name__ == '__main__':
    ...
    for i, finished_task in enumerate(futures_bucket):
        # assumes that all tasks return a dictionary with the 'nodeID', 'workerID', 'start_time', 'stop_time', 'task', 'return_code', and 'output_pdb_file' keys AT LEAST
        results_dictionary = finished_task.result()
        ...
        # if the completed future successfully completed and was a preprocessing task, submit a processing task
        if results_dictionary['return_code'] == 0 and results_dictionary['return_code'] == 'preprocessing':
            futures_bucket.add(client.submit(run_minimization, results_dictionary['output_pdb_file'], results_dictionary['logger_file'], openmm_dictionary = openmm_dictionary, pure = False, resources={'GPU':1}))
        # if the completed future successfully completed and was a processing task, submit a postprocessing task
        elif results_dictionary['return_code'] == 0 and results_dictionary['return_code'] == 'processing':
            futures_bucket.add(client.submit(center, results_dictionary['output_pdb_file'], results_dictionary['logger_file'], pure=False, resources={'CPU':1}))
