...
if __name__ == '__main__':
    ...
    system_descriptor = pdb_file.parent.name
    model_descriptor  = pdb_file.stem
    path = directory / system_descriptor

    # make working directory.
    try:
        print(f'{str(path)}',flush=True)
        os.makedirs(path)
        #os.chmod(path, stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP | stat.S_IROTH)
    except FileExistsError:
        pass

    min_logger = logging_functions.setup_logger('minimization_logger', path / str(model_descriptor + '_min.log'))  # setting up the individual run's logging file
    
    # load pdb file and add missing atoms (mainly hydrogens).
    try:
        start = time.time()
        pdb_file = fix_protein(pdb_file,min_logger,output_pdb_file = path / str(model_descriptor + '_protonated.pdb'))
        ...
        os.chmod(path / str(model_descriptor + '_min.log'), stat.S_IRUSR | stat.S_IWUSR | stat.S_IRGRP | stat.S_IWGRP | stat.S_IROTH)
    ...
    try:
        start = time.time()
        final_pdb = run_minimization(simulation, str(path / model_descriptor), min_logger)
