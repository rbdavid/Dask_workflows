...
## dask scheduler 2 for CPU workers
#jsrun --smpiargs="off" --nrs 1 --rs_per_host 1 --tasks_per_rs 1 --cpu_per_rs 1 --gpu_per_rs 0 --latency_priority cpu-cpu --bind none \
#	--stdio_stdout ${RUN_DIR}/cpu_dask_scheduler.stdout --stdio_stderr ${RUN_DIR}/cpu_dask_scheduler.stderr \
#	dask-scheduler --interface ib0 --no-dashboard --no-show --scheduler-file $CPU_SCHEDULER_FILE &
...
jsrun --smpiargs="off" --nrs 1 --rs_per_host 1 --tasks_per_rs 1 --cpu_per_rs 1 --gpu_per_rs 0 --latency_priority cpu-cpu \
	...
	python3 ${SRC_DIR}/simple.py $SCHEDULER_FILE $SCHEDULER_FILE 
